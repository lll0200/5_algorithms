...
def ok():
    ...
    def selectPath():
        ...
        # print('path',path_,entry.get())

    Label = tk.Label(second, text="文件路径:", font=('黑体', 12))
    Label.place(x=20, y=20)
    entry = tk.Entry(second)
    entry.place(x=100, y=20)
    button = tk.Button(second, text="check", command=selectPath, font=('黑体', 12))
    ...
    button=tk.Button(second,
                     text='ok',
                     command=_main,
                     font=('黑体',15)
                     )
    button.place(x=200,y=180)
    button1 = tk.Button(second,
                       text='关闭',
                       command=re,
                       font=('黑体', 13)
                       )
    ...
    lab2 = tk.Label(second,
                    text="温馨提示！\n需要几分钟的时间\n请耐心等待!",
                    bg="yellow")
    lab2.place(x=280, y=160)
    global text0
    text0 = tk.Text(second,
                    width=40,
                    height=8)
